...
public class BoardSystemServlet extends HttpServlet {
    ...
    private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("/list.do")) {
			...
			if(flag) {
				out.print("<script>alert('글을 수정했습니다.');location.href='list.do';</script>");
			}else {
				out.print("<script>alert('글수정을 실패했습니다.');location.href='updateForm.do?bno="+bno+"';</script>");
			}
		}else if(action.equals("/delete.do")) {
			int bno=Integer.parseInt(request.getParameter("bno"));
			boolean flag=BoardDao.getInstance().deleteBoard(bno);
			if(flag) {
				out.print("<script>alert('글을 삭제했습니다.');location.href='list.do';</script>");
			}else {
				out.print("<script>alert('글삭제를 실패했습니다.');location.href='read.do?bno="+bno+"';</script>");
			}
