...
public class BoardDao {
	...
	public List<Board> selectAll(int start, int end){
		...
		String oracle_sql2= "select * from (select /* +INDEX(BNO_IDX) */ rownum as rn, b1.* from board b1 where rownum<=end) where rn>=start";
		List<Board> list = new ArrayList<Board>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = DBConn.getConn();
			pstmt = conn.prepareStatement(oracle_sql);
